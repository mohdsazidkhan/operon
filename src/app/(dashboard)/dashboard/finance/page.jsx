...
export default async function FinanceDashboard() {
    ...
    const kpis = [
        ...
        { title: 'Neural Margin', value: 195870, change: '+18.2%', up: true, icon: TrendingUp, color: 'text-primary-400', bg: 'bg-primary-500/10' },
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-wrap items-end justify-between gap-6 px-2">
                <div>
                    <h1 className="text-3xl font-black text-white tracking-tighter uppercase italic">Fiscal Matrix</h1>
                ...
                <div className="flex items-center gap-3">
                    <button className="flex items-center gap-2 px-6 py-3 bg-slate-900 border border-slate-800 text-slate-400 hover:text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all">
                        Ledger Verification
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {kpis.map((k, i) => (
                    <div key={i} className="bg-slate-900/40 backdrop-blur-xl rounded-[2.5rem] p-7 border border-slate-800 shadow-2xl relative overflow-hidden group hover:border-slate-700 transition-all">
                        ...
                        <p className="text-3xl font-black text-white tracking-tighter relative z-10">{formatCurrency(k.value)}</p>
                ...
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h3 className="text-sm font-black text-white uppercase tracking-[0.3em] flex items-center gap-2">
                            <BarChart3 size={16} className="text-primary-400" /> P&L Variance
                        </h3>
                        <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mt-1">Revenue Performance vs Operating Overheads</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pb-12">
                <div className="bg-slate-900/40 backdrop-blur-3xl rounded-[3rem] border border-slate-800 p-8 shadow-2xl">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h3 className="text-sm font-black text-white uppercase tracking-[0.3em] flex items-center gap-2">
                                <Activity size={16} className="text-primary-400" /> Cash Propagation
                            </h3>
                            <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mt-1">H1 2024 Liquidity Assessment</p>
                        </div>
                    </div>
                ...
                <div className="bg-slate-900/40 backdrop-blur-3xl rounded-[3rem] border border-slate-800 p-8 shadow-2xl">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mt-1">Expenditure classification breakdown</p>
                        </div>
                    </div>
                    <div className="h-[220px]">
