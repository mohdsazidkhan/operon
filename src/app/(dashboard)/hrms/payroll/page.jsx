    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700 pb-12">
            ...
            <div className="flex flex-wrap items-end justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-white tracking-tighter uppercase italic">Compensation Engine</h1>
                    <div className="flex items-center gap-3 mt-2">
                        <CreditCard size={14} className="text-primary-500" />
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Fiscal Period: March 2024</span>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <button className="flex items-center gap-2 px-5 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all border border-slate-700">
                        <Printer size={16} /> Ledger Export
                    </button>
                    <button className="flex items-center gap-2 px-6 py-3 bg-primary-500 hover:bg-primary-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all shadow-2xl shadow-primary-500/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { label: 'Total Disbursed', value: summary.totalNetPay, color: 'text-primary-400', icon: Wallet, bg: 'from-primary-500/10 to-transparent' },
                    { label: 'Unit Benchmark', value: summary.averageSalary, color: 'text-emerald-400', icon: ArrowUpRight, bg: 'from-emerald-500/10 to-transparent' },
                    { label: 'Recipient Count', value: summary.totalEmployees, color: 'text-indigo-400', icon: UserCircle, bg: 'from-indigo-500/10 to-transparent', suffix: 'Personnel' },
                ].map((stat, i) => (
                    <div key={i} className={cn('bg-slate-900/50 backdrop-blur-sm rounded-[2rem] p-8 border border-slate-800 shadow-2xl relative overflow-hidden group bg-gradient-to-br', stat.bg)}>
                ...
                <div className="p-6 border-b border-white/5 flex flex-wrap items-center justify-between gap-4">
                    <div className="relative flex-1 max-w-sm">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-4 py-4 rounded-3xl bg-slate-950/50 border border-slate-800 text-[10px] font-black uppercase tracking-[0.2em] text-white focus:outline-none focus:ring-2 focus:ring-primary-500/20 transition-all placeholder:text-slate-900"
                        />
                    </div>
                </div>
                                ...
                                <tr key={r._id} className="hover:bg-white/[0.02] transition-colors group">
                                    <td className="py-5 px-8">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-2xl overflow-hidden ring-4 ring-slate-950 shadow-2xl group-hover:scale-105 transition-transform duration-500">
                                                <img src={r.employee?.avatar || 'https://i.pravatar.cc/150'} alt="" className="w-full h-full object-cover text-[8px]" />
                                            </div>
                                            <div>
                                                <p className="text-xs font-black text-white group-hover:text-primary-400 transition-colors uppercase tracking-tight">{r.employee?.name}</p>
                                                <div className="flex items-center gap-1.5 mt-0.5">
                                                    <Briefcase size={10} className="text-slate-600" />
                                                    <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest">{r.employee?.position}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="py-5 px-8 text-sm font-black text-slate-400 text-right">{formatCurrency(r.baseSalary)}</td>
                                    ...
                                    <td className="py-5 px-8 text-right">
                                        <p className="text-base font-black text-white tracking-tighter">{formatCurrency(r.netPay)}</p>
                                        <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mt-0.5 italic">{r.period}</p>
                                    </td>
                                    <td className="py-5 px-8">
                                    ...
                                    <td className="py-5 px-8 text-right">
                                        <button className="p-2.5 rounded-xl hover:bg-slate-800 text-slate-600 hover:text-white transition-all transform group-hover:translate-x-1">
            ...
            {/* Quick Tip */}
            <div className="bg-gradient-to-r from-primary-500/10 via-transparent to-transparent border-l-2 border-primary-500 p-6 rounded-3xl">
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-primary-500/20 flex items-center justify-center text-primary-400">
                        <DollarSign size={16} />
                    </div>
                    <p className="text-[10px] font-black text-primary-400 uppercase tracking-widest leading-relaxed">
                        Payroll execution for <span className="text-white">April 2024</span> is scheduled to trigger in <span className="text-white">6 days</span>. Ensure all attendance vector deltas are normalized before then.
                    </p>
                </div>
            </div>
        </div>
