...
export async function GET(req, { params }) {
    try {
        await dbConnect();
        const b = await Budget.findById((await params).id).populate('approvedBy createdBy', 'name').lean();
...
export async function PUT(req, { params }) {
    try {
        ...
        const b = await Budget.findByIdAndUpdate((await params).id, body, { new: true });
        if (!b) return NextResponse.json({ success: false, message: 'Not found' }, { status: 404 });
        return NextResponse.json({ success: true, data: b });
    } catch (err) {
...
export async function DELETE(req, { params }) {
    try {
        await dbConnect();
        await Budget.findByIdAndDelete((await params).id);
