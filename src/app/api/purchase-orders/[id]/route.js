...
export async function GET(req, { params }) {
    try {
        await dbConnect();
        const po = await PurchaseOrder.findById((await params).id).populate('vendor', 'name code email phone address').lean();
...
export async function PUT(req, { params }) {
    try {
        ...
        const po = await PurchaseOrder.findByIdAndUpdate((await params).id, body, { new: true });
        if (!po) return NextResponse.json({ success: false, message: 'Not found' }, { status: 404 });
        return NextResponse.json({ success: true, data: po });
    } catch (err) {
...
export async function DELETE(req, { params }) {
    try {
        await dbConnect();
        await PurchaseOrder.findByIdAndDelete((await params).id);
