...
export async function GET(req, { params }) {
    try {
        await dbConnect();
        const review = await Performance.findById((await params).id)
...
export async function PUT(req, { params }) {
    try {
        ...
        const review = await Performance.findByIdAndUpdate((await params).id, body, { new: true });
        if (!review) return NextResponse.json({ success: false, message: 'Not found' }, { status: 404 });
        return NextResponse.json({ success: true, data: review });
    } catch (err) {
...
export async function DELETE(req, { params }) {
    try {
        await dbConnect();
        await Performance.findByIdAndDelete((await params).id);
